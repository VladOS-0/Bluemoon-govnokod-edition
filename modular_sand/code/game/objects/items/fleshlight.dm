		...
		switch(targetting)
			if(CUM_TARGET_VAGINA)
				if(!human.has_vagina() == HAS_EXPOSED_GENITAL)
					to_chat(human, "<span class='warning'>Влагалище прикрыто или отсутствует!</span>")
					return FALSE
			if(CUM_TARGET_ANUS)
				if(!human.has_anus() == HAS_EXPOSED_GENITAL)
					to_chat(human, "<span class='warning'>Анус прикрыт или отсутствует!</span>")
					return FALSE
			if(CUM_TARGET_PENIS)
				if(!human.has_penis() == HAS_EXPOSED_GENITAL && !human.has_strapon() == HAS_EXPOSED_GENITAL)
					to_chat(human, "<span class='warning'>Пенис прикрыт или отсутствует!</span>")
					return FALSE
			if(CUM_TARGET_URETHRA)
				if(!human.has_penis() == HAS_EXPOSED_GENITAL && !human.has_strapon() == HAS_EXPOSED_GENITAL)
					to_chat(human, "<span class='warning'>Уретра прикрыта или отсутствует!</span>")
					return FALSE
			if(CUM_TARGET_MOUTH)
