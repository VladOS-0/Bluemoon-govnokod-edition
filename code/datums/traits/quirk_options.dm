...
/datum/quirk_option/proc/list_options(user)
...
/datum/quirk_option/proc/prefs_convert(option)
	if(option && quirk.name && name)
		var/pref_string = quirk.name + QUIRK_OPTION_STRING_SEPARATOR + name + QUIRK_OPTION_STRING_SEPARATOR + option + QUIRK_OPTION_STRING_SEPARATOR + opt_version
		return pref_string
	return FALSE
...
/datum/quirk_option/proc/find_n_replace(list/all_options, new_string)
	...
	if(!new_option_parameters || new_option_parameters.len < 4)
